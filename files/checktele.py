...
def gen_user(choice):
    if choice == "1" or choice == "ثلاثي_مختلط":
        ...
        if username in banned[0]:
            english_words_list = list(filter(lambda x: len(x) == 8, english_words))
            username = random.choice(english_words_list)
    elif 'X' in choice or 'A' in choice or 'Y' in choice or 'B' in choice or '1' in choice or '2' in choice:
        modified_word = ""
        for char in word:
            aa = (''.join(random.choice(a)for i in range(1)))
            bb = (''.join(random.choice(a)for i in range(1)))
            yy = (''.join(random.choice(e)for i in range(1)))
            xx = (''.join(random.choice(e)for i in range(1)))
            nu1 = (''.join(random.choice(b)for i in range(1)))
            nu2 = (''.join(random.choice(b)for i in range(1)))
